...
def run_simulation(ndays,npeople,locations,loc_weights=None,init_locations=None,init_states=None,lunchtimes=None):
    """
    ...
    for day_idx in tqdm(range(ndays),desc='Running Simulation'):  #loop over days
        ...
        testvec=np.zeros(npeople)  # a vector holding the indices of people to test
        testarray = np.zeros([11,npeople])  # a vector holding the state of tested people
        #place quarantine policy here
        ...
        for time_idx in range(1,96): #loop over small time interval (now 5 minutes.)
            ...
            for person_idx in range(npeople):
                ...
                agent_states = transmission(prev_idx,current_idx,person_idx,locations,agent_locations,current_location,agent_states,loc_weights,lambda_=1e19)
            
            prev_idx = current_idx
            current_time+=1
        
        #place testing policy here
...
def forecast(ntrials,prev_locations,locations,init_state,loc_weights,known_indices=[]):
    ...
    for person_idx in unknown_indices:#range(prev_locations.shape[-1]):
